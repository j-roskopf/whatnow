...
function mergeRatings(igdb: GameRatings, openCritic?: RatingScore | null): GameRatings {
...
export async function lookupGameMeta(
	...
	const [sgdbCover, igdbGame, openCritic] = await Promise.all([
	...
	const ratings = mergeRatings(igdbGame ? ratingsFromIgdb(igdbGame) : { scores: [] }, openCritic);
...
export async function lookupGameRatings(
	...
	for (const lookup of lookups.slice(0, 48)) {
		const meta = await lookupGameMeta(lookup.name, keys, { searchAs: lookup.searchAs });
		if (meta.ratings.scores.length) ratings[lookup.id] = meta.ratings;
	}
